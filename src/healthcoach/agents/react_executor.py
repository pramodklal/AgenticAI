...
def _lookup_doctors_npi(zip_code: str, specialty: str, limit: int = 3) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        for record in data.get("results", [])[:limit]:
            ...
            primary_taxonomy = next((t.get("desc", specialty) for t in taxonomies if t.get("primary")), specialty)
            results.append({
                "name": f"{basic.get('first_name', '')} {basic.get('last_name', '')}".strip() or basic.get("organization_name", "Unknown"),
                "specialty": primary_taxonomy,
                "address": f"{practice.get('address_1', '')} {practice.get('city', '')} {practice.get('state', '')} {practice.get('postal_code', '')}".strip(),
...
def _lookup_doctors_osm(lat: float, lon: float, specialty: str, limit: int = 3) -> List[Dict[str, Any]]:
    """Query Overpass API for clinics/doctors near a lat/lon point."""
    radius = 15000  # 15 km
    overpass_query = f"""
[out:json][timeout:10];
(
    ...
    try:
        ...
        for element in data.get("elements", []):
            ...
            if not name or name in seen:
                continue
            seen.add(name)
            # derive lat/lon for ways (centre point)
...
def run_supervisor_react(state: HealthCoachState) -> Dict[str, Any]:
    ...
    system_prompt = (
        ...
        "Based on the user's question, consult the relevant domain specialists, then provide a unified, "
        "practical response that integrates their advice. "
        "Be thorough — consult all domains relevant to the question. "
